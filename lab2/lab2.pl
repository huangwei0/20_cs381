...
sub mcw{
	...
	##########################
	##                      ##
	#reference:https://www.geeksforgeeks.org/perl-multidimensional-hashes/
	#          https://perldoc.perl.org/functions/sort.html
	$stric = 0;
	for my $key (keys %counts){
		if($word eq $key){
			for my $next (sort(keys %{$counts{$key}})){
				if(exists ($word_history{$next})){
					next;
				}
				else{
					if($counts{$key}{$next}> $stric){
						$stric = $counts{$key}{$next};
						$best_word = $next;
					}
				}
			}
...
sub sequence{
	...
	$word_history{$word} = "skip";
	push @sequence, $word;
	while(($count < $SEQUENCE_LENGTH) && ($word ne "") && ($word ne "\n" )){
		$next = mcw($word);
		push @sequence, $next;
		$word_history{$next} = "skip";
		$word = $next;
		$count++;
	}
