...
leftmost :: Tree -> Int
leftmost (Leaf a)     = a
leftmost (Node _ b _) = leftmost b
...
rightmost :: Tree -> Int
rightmost (Leaf a) = a 
rightmost (Node _ _ b) = rightmost b
...
isBST :: Tree -> Bool
isBST (Leaf a) = True
isBST (Node a b c) = ((valBST b) < a) && (isBST b) && ((valBST c) > a) && (isBST c)
...
inBST :: Int -> Tree -> Bool
inBST a (Leaf b) = a == b
inBST a (Node b c d) = a == b || inBST a c || inBST a d